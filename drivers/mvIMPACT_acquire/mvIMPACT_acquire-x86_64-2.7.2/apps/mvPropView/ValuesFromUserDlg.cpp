...
//-----------------------------------------------------------------------------
{
    ...
    if( boCreateApplyButton )
    {
        ...
        pButtonSizer->Add( pBtnApply_, wxSizerFlags().Right().Border( wxALL, 7 ) );
    }
    pSizer->AddSpacer( 10 );
...
//-----------------------------------------------------------------------------
{
    ...
    */
    ...
    wxFlexGridSizer* pGridSizer = new wxFlexGridSizer( 2, colCount );
...
//-----------------------------------------------------------------------------
{
    ...
    */
    ...
    else if( type & ctProp )
    {
        ...
        AddFeatureInfo( wxT( "Property Value Count(max): " ), wxString::Format( wxT( "%d" ), prop.maxValCount() ) );
        if( ( prop.type() == ctPropString ) && ( prop.flags() & cfContainsBinaryData ) )
        {
            ...
            {
                wxString binaryDataRAW( ConvertedString( BinaryDataToString( PropertyS( prop ).readBinary( i ) ) ) );
...
//-----------------------------------------------------------------------------
{
    ...
    */
    ...
    pTCBinaryData_ = new wxTextCtrl( pPanel, widTCBinaryData, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxSUNKEN_BORDER | wxTE_RICH2, HEXStringValidator_ );
    pTCBinaryData_->SetDefaultStyle( fixedPitchStyle_ );
    pTCAsciiData_ = new wxTextCtrl( pPanel, widTCAsciiData, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxSUNKEN_BORDER | wxTE_RICH2 | wxTE_READONLY );
...
AssignSettingsToDisplaysDlg::AssignSettingsToDisplaysDlg( wxWindow* pParent, const wxString& title,
        const std::vector<std::pair<std::string, int> >& settings, const SettingToDisplayDict& settingToDisplayDict, size_t displayCount )
    : OkAndCancelDlg( pParent, wxID_ANY, title )
//-----------------------------------------------------------------------------
{
    /*
    ...
    */
    ...
    {
        choices.Add( wxString::Format( wxT( "Display %d" ), i ) );
    }
    for( int i = 0; i < colCount; i++ )
    {
