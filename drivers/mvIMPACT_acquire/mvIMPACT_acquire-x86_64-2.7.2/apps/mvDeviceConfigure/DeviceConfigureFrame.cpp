                       ...
                       ( ( pDev = m_devMgr.getDeviceByProduct( it->second.searchToken_, i ) ) != 0 ) )
                {
                    if( !m_IPv4Mask.empty() )
                    {
                        ...
                        if( deviceIPAddress.isValid() )
                        {
                            ...
                            if( ( IPAddress != string( "Unavailable" ) ) && ( match( IPAddress, m_IPv4Mask, '*' ) != 0 ) )
                            {
                    ...
                    {
                        const int result = SetID( pDev, it->second.deviceID_ );
                    ...
                    {
                        const int result = UpdateKernelDriver( pDev, true );
                        if ( result != DeviceHandler::urOperationSuccessful )
                        {
                            m_updateResult = result;
                        }
                    ...
                    {
                        const int result = UpdateFirmware( pDev, true );
                        if( result != DeviceHandler::urOperationSuccessful )
                        {
                            m_updateResult = result;
                        }
