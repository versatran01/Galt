...
PlotCanvasImageAnalysis::PlotCanvasImageAnalysis( wxWindow* parent, const wxString& configName, wxWindowID id /* = -1 */, const wxPoint& pos /* = wxDefaultPosition */, const wxSize& size /* = wxDefaultSize */,
        long style /* = wxSUNKEN_BORDER */, const wxString& name /* = "DrawingCanvas" */, bool boActive /* = false */, int channelCount /* = 0 */ ) :
...
//-----------------------------------------------------------------------------
{
    ...
    if( ( w > 0 ) && ( m_AOIw != w ) )
    {
        ...
        // if the AOI width is larger then x-offset + image width, the AOI width must be clipped
    ...
    if( ( h > 0 ) && ( m_AOIh != h ) )
    {
        ...
        // if the AOI height is larger then y-offset + image height, the AOI height must be clipped
        // this might have been the case in the previous image as well and thus doesn't
        // implies that this is definitely a change -> check twice
        if( newValue != m_AOIh )
        {
