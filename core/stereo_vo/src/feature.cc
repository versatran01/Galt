...
bool Feature::triangulate(const image_geometry::StereoCameraModel &model, scalar_t eigen_threshold) {
  ...
  kr::vec2<scalar_t> lPt((p_pixel_left_.x - lcx)/lfx,
                         (p_pixel_left_.y - lcy)/lfy);
