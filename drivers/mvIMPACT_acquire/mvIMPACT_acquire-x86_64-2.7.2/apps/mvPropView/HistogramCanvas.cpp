...
HistogramCanvas::HistogramCanvas( wxWindow* parent, const wxString& configName, wxWindowID id /* = -1 */, const wxPoint& pos /* = wxDefaultPosition */,
                                  const wxSize& size /* = wxDefaultSize */, long style /* = wxSUNKEN_BORDER */, const wxString& name /* = wxT("Histogram") */, bool boActive /* = false */, bool boHandleStandardDeviation /* = false */ ) :
...
//-----------------------------------------------------------------------------
{
    ...
    case ibpfYUV422Packed:
        CalculateSpatialNoiseHistogramYUV<unsigned char>( pIB );
        break;
    case ibpfYUV422_10Packed:
        CalculateSpatialNoiseHistogramYUV<unsigned short>( pIB );
        break;
    case ibpfYUV422_UYVYPacked:
        CalculateSpatialNoiseHistogramUYV<unsigned char>( pIB );
        break;
    case ibpfYUV422_UYVY_10Packed:
        CalculateSpatialNoiseHistogramUYV<unsigned short>( pIB );
        break;
...
//-----------------------------------------------------------------------------
template<typename _Ty>
void HistogramCanvas::CalculateSpatialNoiseHistogramUYV( const ImageBuffer* pIB )
//-----------------------------------------------------------------------------
{
#ifdef _OPENMP
    #   pragma omp parallel for
...
//-----------------------------------------------------------------------------
template<typename _Ty>
void HistogramCanvas::CalculateSpatialNoiseHistogramYUV( const ImageBuffer* pIB )
//-----------------------------------------------------------------------------
{
#ifdef _OPENMP
    #   pragma omp parallel for
    ...
    for( int line = 0; line < m_AOIh; line++ )
    {
        ...
        for( int pixel = 0; pixel < m_AOIw / 2; pixel++ )
        {
            ...
            if( line < m_AOIh - 1 )
            {
                ...
                ++m_ppHistogramBuffer[pChannel0Ver][difference];
                difference = abs( pData[1] - pData[1 + pIB->pChannels[0].iLinePitch] );
                m_pHistogramAverage[boIsYV ? pChannel2Ver : pChannel1Ver] += difference;
                ++m_ppHistogramBuffer[boIsYV ? pChannel2Ver : pChannel1Ver][difference];
                difference = abs( pData[2] - pData[2 + pIB->pChannels[0].iLinePitch] );
                m_pHistogramAverage[pChannel0Ver] += difference;
                ++m_ppHistogramBuffer[pChannel0Ver][difference];
                difference = abs( pData[3] - pData[3 + pIB->pChannels[0].iLinePitch] );
                m_pHistogramAverage[boIsYV ? pChannel1Ver : pChannel2Ver] += difference;
                ++m_ppHistogramBuffer[boIsYV ? pChannel1Ver : pChannel2Ver][difference];
            }
            pData += 4;
        }
    }
...
//-----------------------------------------------------------------------------
{
    ...
    case ibpfRGBx888Planar:
        ...
        break;
    case ibpfYUV422Packed:
        CalculatePixelHistogramYUV<unsigned char>( pIB );
        break;
    case ibpfYUV422_10Packed:
        CalculatePixelHistogramYUV<unsigned short>( pIB );
        break;
    case ibpfYUV422_UYVYPacked:
        CalculatePixelHistogramUYV<unsigned char>( pIB );
        break;
    case ibpfYUV422_UYVY_10Packed:
        CalculatePixelHistogramUYV<unsigned short>( pIB );
        break;
    case ibpfYUV444_UYVPacked:
...
//-----------------------------------------------------------------------------
template<typename _Ty>
void HistogramCanvas::CalculatePixelHistogramUYV( const ImageBuffer* pIB )
//-----------------------------------------------------------------------------
{
#ifdef _OPENMP
    #   pragma omp parallel for
...
//-----------------------------------------------------------------------------
template<typename _Ty>
void HistogramCanvas::CalculatePixelHistogramYUV( const ImageBuffer* pIB )
//-----------------------------------------------------------------------------
{
#ifdef _OPENMP
    #   pragma omp parallel for
    ...
    for( int line = 0; line < m_AOIh; line++ )
    {
        ...
        for( int pixel = m_AOIx; pixel < LOOP_END; pixel++ )
        {
            ...
            if( pixel % 2 )
            {
                m_pHistogramAverage[pV] += *pData;
                ++m_ppHistogramBuffer[pV][*pData];
                m_pHistogramAverage[pV] += *pData;
                ++m_ppHistogramBuffer[pV][*pData++];
            }
            else
            {
                m_pHistogramAverage[pU] += *pData;
                ++m_ppHistogramBuffer[pU][*pData];
                m_pHistogramAverage[pU] += *pData;
                ++m_ppHistogramBuffer[pU][*pData++];
            }
        }
...
//-----------------------------------------------------------------------------
void HistogramCanvas::PrepareHistogramBuffer( bool boFormatChanged, int channelCount, mvIMPACT::acquire::TImageBufferPixelFormat pixelFormat )
//-----------------------------------------------------------------------------
{
