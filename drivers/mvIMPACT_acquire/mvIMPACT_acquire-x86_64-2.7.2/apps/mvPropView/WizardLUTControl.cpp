...
DEFINE_EVENT_TYPE( GammaParameterChangeEvent )
...
WizardLUTCanvas::WizardLUTCanvas( wxWindow* parent, wxWindowID id /* = -1 */, const wxPoint& pos /* = wxDefaultPosition */,
                                  const wxSize& size /* = wxDefaultSize */, long style /* = wxSUNKEN_BORDER */, const wxString& name /* = "LUT" */, bool boActive /* = false */ )
...
//-----------------------------------------------------------------------------
{
    ...
    */
    ...
    // lower log window
    pLogWindow_ = new wxTextCtrl( pPanel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxSUNKEN_BORDER | wxTE_RICH | wxTE_READONLY );
...
//-----------------------------------------------------------------------------
{
    wxFileDialog fileDlg( this, wxT( "Select a filename to store the current LUT to" ), wxT( "" ), wxT( "" ), wxT( "CSV Files (*.csv)|*.csv" ), wxSAVE | wxOVERWRITE_PROMPT );
...
//-----------------------------------------------------------------------------
{
    wxFileDialog fileDlg( this, wxT( "Select a file to load the current LUT from" ), wxT( "" ), wxT( "" ), wxT( "CSV Files (*.csv)|*.csv" ), wxOPEN | wxFILE_MUST_EXIST );
    if( fileDlg.ShowModal() == wxID_OK )
    {
        ...
        char* pBuf = new char[file.Length() + 1];
        memset( pBuf, 0, file.Length() + 1 );
        const size_t bytesRead = file.Read( pBuf, file.Length() );
        if( bytesRead == static_cast<size_t>( file.Length() ) )
        {
            const wxString fileContent = wxString( ConvertedString( pBuf ) );
            vector<wxString> lines;
            vector<wxString>::size_type lineCount = Split( fileContent, wxT( "\n" ), lines );
            const size_t elementsToProcess = ( activeLUT_.size() < lineCount ) ? activeLUT_.size() : lineCount;
        ...
        {
            wxString msg;
            msg << wxT( "Error detected during LUT import: Bytes read: " ) << bytesRead << wxT( ", expected: " ) << file.Length() << wxT( ".\n" );
