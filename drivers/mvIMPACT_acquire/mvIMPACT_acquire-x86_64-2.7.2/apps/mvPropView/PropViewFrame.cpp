...
DEFINE_EVENT_TYPE( eventNotified )
...
BEGIN_EVENT_TABLE( PropViewFrame, PropGridFrameBase )
    ...
    EVT_COMMAND( widMainFrame, eventNotified, PropViewFrame::OnEventNotified )
...
PropViewFrame::PropViewFrame( const wxString& title, const wxPoint& pos, const wxSize& size, int argc, wxChar** argv )
    : PropGridFrameBase( widMainFrame, title, pos, size ), m_boCloseDeviceInProgress( false ), m_boFindFeatureMatchCaseActive( false ),
      m_boDisplayWindowMaximized( false ), m_boShutdownInProgress( false ), m_boSingleCaptureInProgess( false ),
...
//-----------------------------------------------------------------------------
{
    ...
    */
    ...
    //WriteLogMessage( wxString::Format( wxT("processed command line parameters: %s\n"), ( processedParameters.length() > 0 ) ? processedParameters.c_str() : wxT("none") ) ); // will cause a 'deprecated conversion from string constant to 'char*' on some platform/wxWidgets combinations
    WriteLogMessage( wxT( "\n" ) );
...
//-----------------------------------------------------------------------------
void PropViewFrame::OnEventNotified( wxCommandEvent& e )
//-----------------------------------------------------------------------------
{
    Device* pDevice = static_cast<Device*>( e.GetClientData() );
    if( pDevice )
    {
        WriteLogMessage( wxString::Format( wxT( "There has been a %s event reported by device %s.\n" ), e.GetString().c_str(), ConvertedString( pDevice->serial.read() ).c_str() ), wxTextAttr( wxColour( 0, 150, 50 ) ) );
    }
}
...
//-----------------------------------------------------------------------------
void PropViewFrame::OnImageReady( wxCommandEvent& )
//-----------------------------------------------------------------------------
{
    m_boCurrentImageIsFromFile = false;
...
//-----------------------------------------------------------------------------
{
    ...
    boActive = pConfig->Read( wxT( "/MainFrame/Settings/displayPropGrid" ), 1l ) != 0;
    ...
    boActive = pConfig->Read( wxT( "/MainFrame/Settings/showLeftToolBar" ), 1l ) != 0;
...
//-----------------------------------------------------------------------------
{
    // To prevent a crash when the handler OnImageReady() is called and m_pDevPropHandler is already deleted.
    if( !m_pDevPropHandler )
    {
        return;
    }
...
//-----------------------------------------------------------------------------
{
    ...
    if( devCnt > 0 )
    {
        for( unsigned int i = 0; i < devCnt; i++ )
        {
            if( !boDisplayConnectedDevicesOnly || ( devMgr[i]->state.read() == dsPresent ) )
            {
...
//-----------------------------------------------------------------------------
{
    ...
    if( boValidDevice || boValidImage )
    {
        ...
        // pCT ? pCT->GetLastRequestResult().c_str() : wxT("loaded from disc") )); // will cause a 'deprecated conversion from string constant to 'char*' on some platform/wxWidgets combinations
