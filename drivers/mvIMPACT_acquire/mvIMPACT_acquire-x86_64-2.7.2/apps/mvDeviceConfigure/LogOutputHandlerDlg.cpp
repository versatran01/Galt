...
//-----------------------------------------------------------------------------
{
    ...
    // list control
    m_pLogListCtrl = new LogOutputListCtrl( pHorizontalSplitter, LIST_CTRL, wxDefaultPosition, wxDefaultSize, wxLC_REPORT | wxLC_SINGLE_SEL | wxSUNKEN_BORDER, this );
    ...
    pButtonSizer->Add( m_pBtnOpen, wxSizerFlags().Border( wxALL, 7 ).Right() );
    m_pBtnSave = new wxButton( pPanel, widBtnSave, wxT( "&Save..." ) );
    pButtonSizer->Add( m_pBtnSave, wxSizerFlags().Border( wxALL, 7 ).Right() );
...
//-----------------------------------------------------------------------------
{
    wxFileDialog fileDlg( this, wxT( "Load an existing log configuration" ), wxT( "" ), wxT( "" ), wxT( "log-config Files (*.mvd)|*.mvd" ), wxOPEN | wxFILE_MUST_EXIST );
...
//-----------------------------------------------------------------------------
{
    ...
    wxFileDialog fileDlg( this, wxT( "Save the current log configuration" ), wxT( "" ), wxT( "" ), wxT( "log-config Files (*.mvd)|*.mvd" ), wxSAVE | wxOVERWRITE_PROMPT );
    if( fileDlg.ShowModal() == wxID_OK )
    {
        wxString pathName = fileDlg.GetPath();
