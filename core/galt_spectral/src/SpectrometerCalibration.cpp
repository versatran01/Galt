...
bool YAML::convert<galt::SpectrometerCalibration>::decode(
  ...
  const auto requiredFields = { "calibration_date", "source_spectrum", "sample_spectra"};
...
YAML::Emitter &operator<<(YAML::Emitter &out,
  ...
  for (size_t i=0;i < calib.sampleSpectra.size(); i++) {
    out << YAML::BeginMap;
    out << YAML::Key << "reflectance" << YAML::Value << calib.sampleSpectra[i].reflectance;
    out << YAML::Key << "measurement" << YAML::Value << calib.sampleSpectra[i].measurement;
    out << YAML::EndMap;
  }
  out << YAML::EndSeq;
  out << YAML::EndMap;
