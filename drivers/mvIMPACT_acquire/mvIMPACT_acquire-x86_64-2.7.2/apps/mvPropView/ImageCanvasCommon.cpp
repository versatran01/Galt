...
//-----------------------------------------------------------------------------
{
    _Ty* p = reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pIB->vpData ) + ( pixel.y * pIB->pChannels[0].iLinePitch ) + ( pixel.x * pIB->iBytesPerPixel ) );
    pixelInfo.append( wxString::Format( wxT( "%d(Y), %d(U), %d(V)" ), p[order[0]], p[order[1]], p[order[2]] ) );
}

//-----------------------------------------------------------------------------
template<typename _Ty>
void ImageCanvas::AppendYUVDataPixelInfo( wxPoint pixel, wxString& pixelInfo, const ImageBuffer* pIB ) const
//-----------------------------------------------------------------------------
{
    _Ty* p = reinterpret_cast<_Ty*>( reinterpret_cast<unsigned char*>( pIB->vpData ) + ( pixel.y * pIB->pChannels[0].iLinePitch ) + ( pixel.x * pIB->iBytesPerPixel ) );
...
//-----------------------------------------------------------------------------
{
    ...
    case ibpfMono16:
        {
            unsigned short* p = reinterpret_cast<unsigned short*>( reinterpret_cast<unsigned char*>( m_pIB->vpData ) + ( pixel.y * m_pIB->pChannels[0].iLinePitch ) ) + pixel.x;
            pixelInfo.append( wxString::Format( wxT( "%d" ), *p ) );
        }
        break;
    case ibpfMono12Packed_V1:
        pixelInfo.append( wxString::Format( wxT( "%d" ), PlotCanvasImageAnalysis::GetMono12Packed_V1Pixel( static_cast<unsigned char*>( m_pIB->vpData ), ( pixel.y * m_pIB->iWidth ) + pixel.x ) ) );
        break;
    case ibpfMono12Packed_V2:
        pixelInfo.append( wxString::Format( wxT( "%d" ), PlotCanvasImageAnalysis::GetMono12Packed_V2Pixel( static_cast<unsigned char*>( m_pIB->vpData ), ( pixel.y * m_pIB->iWidth ) + pixel.x ) ) );
        break;
    case ibpfRGB101010Packed:
    case ibpfRGB121212Packed:
    case ibpfRGB141414Packed:
    case ibpfRGB161616Packed:
        {
            unsigned short* p = reinterpret_cast<unsigned short*>( reinterpret_cast<unsigned char*>( m_pIB->vpData ) + ( pixel.y * m_pIB->pChannels[0].iLinePitch ) + ( pixel.x * m_pIB->iBytesPerPixel ) );
            pixelInfo.append( wxString::Format( wxT( "%d(R), %d(G), %d(B)" ), p[2], p[1], p[0] ) );
        }
        break;
    case ibpfBGR888Packed:
        {
            unsigned char* p = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + ( pixel.y * m_pIB->pChannels[0].iLinePitch ) + ( pixel.x * m_pIB->iBytesPerPixel );
    ...
    case ibpfBGR101010Packed_V2:
        {
            ...
            unsigned int* p = reinterpret_cast<unsigned int*>( reinterpret_cast<unsigned char*>( m_pIB->vpData ) + ( pixel.y * m_pIB->pChannels[0].iLinePitch ) ) + pixel.x;
    ...
    case ibpfRGBx888Planar:
        {
            unsigned char* r = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + m_pIB->pChannels[0].iChannelOffset + ( pixel.y * m_pIB->pChannels[0].iLinePitch ) + pixel.x;
            unsigned char* g = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + m_pIB->pChannels[1].iChannelOffset + ( pixel.y * m_pIB->pChannels[1].iLinePitch ) + pixel.x;
            unsigned char* b = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + m_pIB->pChannels[2].iChannelOffset + ( pixel.y * m_pIB->pChannels[2].iLinePitch ) + pixel.x;
            pixelInfo.append( wxString::Format( wxT( "%d(R), %d(G), %d(B)" ), *r, *g, *b ) );
        }
        break;
    case ibpfMono8:
        {
            unsigned char* p = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + ( pixel.y * m_pIB->pChannels[0].iLinePitch ) + pixel.x;
            pixelInfo.append( wxString::Format( wxT( "%d" ), *p ) );
        }
        break;
    case ibpfYUV422Packed:
        AppendYUVDataPixelInfo<unsigned char>( pixel, pixelInfo, m_pIB );
        break;
    case ibpfYUV422_10Packed:
        AppendYUVDataPixelInfo<unsigned short>( pixel, pixelInfo, m_pIB );
        break;
    case ibpfYUV422_UYVYPacked:
    ...
    case ibpfYUV422Planar:
        {
            unsigned char* y = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + m_pIB->pChannels[0].iChannelOffset + ( pixel.y * m_pIB->pChannels[0].iLinePitch ) + pixel.x;
            unsigned char* u = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + m_pIB->pChannels[1].iChannelOffset + ( pixel.y * m_pIB->pChannels[1].iLinePitch ) + pixel.x / 2;
            unsigned char* v = reinterpret_cast<unsigned char*>( m_pIB->vpData ) + m_pIB->pChannels[2].iChannelOffset + ( pixel.y * m_pIB->pChannels[2].iLinePitch ) + pixel.x / 2;
...
//-----------------------------------------------------------------------------
{
    ...
    e.Skip();
    wxCommandEvent eDisplaySelection( imageCanvasSelected, m_pApp->GetId() );
    eDisplaySelection.SetInt( m_userData );
    ::wxPostEvent( m_pApp->GetEventHandler(), eDisplaySelection );
}
...
//-----------------------------------------------------------------------------
void ImageCanvas::SetInfoOverlay( const std::vector<wxString>& infoStrings )
//-----------------------------------------------------------------------------
{
