...
//-----------------------------------------------------------------------------
{
    ...
    result = DMR_AcquisitionStart( hDrv );
    ...
    // Whether this is needed or not depends on the property 'AcquisitionStartStopBehaviour' in the list referenced by 'HDEV'.
    result = DMR_AcquisitionStop( hDrv );
    if( ( result != DMR_NO_ERROR ) &&
        ( result != DMR_FEATURE_NOT_AVAILABLE ) )
    {
...
//-----------------------------------------------------------------------------
{
    ...
    if( ( hPropInterfaceLayout = getDeviceProp( hDevice, "InterfaceLayout" ) ) != INVALID_ID )
    {
        conditionalSetPropI( hPropInterfaceLayout, dilGenICam );
    }
    ...
    if( result == DMR_NO_ERROR )
    {
        ...
        if( ( result == DMR_NO_ERROR ) && ( ReqRes.result == rrOK ) )
        {
            ...
            if( result != 0 )
            {
                printf( "An error occurred while setting up the user supplied buffers(error code: %s). Press [ENTER] to end the application...\n", DMR_ErrorCodeToString( result ) );
                END_APPLICATION;
            }
        }
        else
    ...
    // requested by the user. However sometimes it can be neccessary to make sure that a
    ...
    // define that 'REQUEST_TO_USE' is used for the next acquisiton
