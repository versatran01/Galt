...
//-----------------------------------------------------------------------------
{
    if( boOn )
    {
            ...
            ( ( result == DEV_NO_FREE_REQUEST_AVAILABLE ) && ( framesRequested > 0 ) ) )
        {
            ...
            default:
                wxMessageDialog dlg( NULL, wxString::Format( wxT( "Live not started(Reason: %s)! More information can be found in the *.log-file or the debug output.\n" ), ConvertedString( ImpactAcquireException::getErrorCodeAsString( result ) ).c_str() ), wxT( "Error" ), wxOK | wxICON_WARNING );
                dlg.ShowModal();
                break;
            }
        }
